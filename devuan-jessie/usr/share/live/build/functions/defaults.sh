...
Set_defaults ()
{
	...
	# Setting mode (currently: debian, progress-linux, ubuntu, and devuan)
	if [ -x /usr/bin/lsb_release ]
	then
		_DISTRIBUTOR="$(lsb_release -is | tr "[A-Z]" "[a-z]")"

		case "${_DISTRIBUTOR}" in
			debian|progress-linux|ubuntu|devuan)
	...
	case "${LB_MODE}" in
		...
		devuan)
			# jessie alpha version has some problems with
			# GTK deps otherwise
			LB_APT_RECOMMENDS="${LB_APT_RECOMMENDS:-false}"
			;;
		...
		devuan)
			case "${LB_DISTRIBUTION}" in
				jessie*)
					LB_INITSYSTEM="${LB_INITSYSTEM:-sysvinit}"
					;;
				*) # who knows? s6? runit? we shall see :-)
					LB_INITSYSTEM="${LB_INITSYSTEM:-sysvinit}"
					;;
			esac
			;;
	...
	case "${LB_MODE}" in
		...
		devuan) # N.B. this should probably be changed once devuan has a security repository
			LB_MIRROR_CHROOT_SECURITY="${LB_MIRROR_CHROOT_SECURITY:-http://security.debian.org/}"
			LB_PARENT_MIRROR_CHROOT_SECURITY="${LB_PARENT_MIRROR_CHROOT_SECURITY:-${LB_MIRROR_CHROOT_SECURITY}}"
			;;
	...
	case "${LB_MODE}" in
		...
		devuan)
			LB_MIRROR_BINARY="${LB_MIRROR_BINARY:-http://packages.devuan.net/merged}"
			LB_PARENT_MIRROR_BINARY="${LB_PARENT_MIRROR_BINARY:-${LB_MIRROR_BINARY}}"
			;;
	...
	case "${LB_MODE}" in
		...
		devuan)
			LB_KEYRING_PACKAGES="${LB_KEYRING_PACKAGES:-devuan-keyring}"
			;;
	...
	case "${LB_MODE}" in
		...
		devuan)
			LB_ISO_APPLICATION="${LB_ISO_APPLICATION:-Devuan Live}"
			;;
	esac
	...
	case "${LB_MODE}" in
		...
		devuan)
			LB_HDD_LABEL="${LB_HDD_LABEL:-DEVUAN_LIVE}"
			;;
	esac
	...
	case "${LB_MODE}" in
		...
		devuan)
			LB_ISO_VOLUME="${LB_ISO_VOLUME:-Devuan ${LB_DISTRIBUTION} \$(date +%Y%m%d-%H:%M)}"
			;;
	...
	## download Packages file instead of reconstructing it
	case "${LB_MODE}" in
		devuan)
		LB_BINARY_USE_ORIGINAL_PACKAGES_LIST="${LB_BINARY_USE_ORIGINAL_PACKAGES_LIST:-true}"
		;;

		*)
		LB_BINARY_USE_ORIGINAL_PACKAGES_LIST="${LB_BINARY_USE_ORIGINAL_PACKAGES_LIST:-false}"
		;;
	esac
}
